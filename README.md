...
원본 엑셀(`참고자료/■중요■POI_최종(260723).xlsx`, POI 약 8,247행·와이드)을 집계하여 `data/`에 JSON으로 생성합니다.
...
> **탄소배출량(tCO₂e) = Σ(월별 방문자수 × 계수(B안) × 업종가중치 × EWrt) / 1000**  
> 계수·가중치·EWrt는 원본 엑셀 POI행 속성을 사용합니다. UI의 `pc`는 실효 1인당 kg(총배출kg/총방문자)입니다.

### 데이터 범위 (산출 결과)

- POI 8,246개 · 17개 시도 · 252개 시군구
- 기간 2023.01 ~ 2026.04 (40개월)
- 대분류 9종 · 중분류 38종  
- 방문객 유형(현지인/외지인): 신규 원본에 유형 분리가 없어 UI 필터는 제거함. 스키마 호환용으로 전체 방문은 `v`/`vL`에 귀속, `vO`=0
