...
### [2026-07-24] POI_최종(260723) 엑셀 데이터 교체 (UI 무변경)

**요청**: 서비스 플로우·UI 유지, 원본 엑셀만 신규 파일로 교체. 커밋/푸시 없음.

**변경**:
- `scripts/build_data.py` — 와이드 포맷(`POI_탄소발자국`) 파싱, 배출=`방문자×계수×가중치×EWrt`
- 스키마 호환: `vL`=전체·`vO`=0, `scls`=중분류, 기존 `cont_id` 이름매칭 유지
- `data/*.json` 재생성 (POI 8,247 / 202301~202604)
- README 원본·기간·산식 문구 갱신

**파일**: `scripts/build_data.py`, `data/*`, `README.md`, `HISTORY.md`

**키워드**: `엑셀`, `build_data`, `POI_최종`, `데이터교체`

**비고**: 방문객 유형 필터에서 「외지인」은 데이터 부재로 0. 월별 탄소 수식 컬럼은 캐시값 없어 속성으로 재계산.

---

### [2026-07-25] P3 포지셔닝 산점도 UI 개선

**요청**: 「POI 인기(방문자) 대비 탄소배출 포지셔닝」 섹션 판독성 개선.

**문제**: 1인당 배출량이 롱테일(중앙값 0.56 / 최대 250)이라 선형 Y축에서 점이 바닥에 눌려 판독 불가. 표시 대상도 배출량 상위 900개로 잘려 분포가 왜곡됨.

**변경**:
- Y축 로그 스케일 전환, 양축 범위를 1·2·5×10ⁿ 경계로 정규화
- 사분면 배경 음영 + 코너 라벨, 카드 하단에 사분면별 건수 칩 추가
- 대분류별 시리즈 분리 → 범례 토글 필터, hover 시 시리즈 강조/나머지 흐림
- 필터 결과 전량 표시(상위 900 제한 제거), 버블 크기를 로그 스케일로 압축
- 툴팁에 사분면 판정·단위 포맷 추가, Ctrl+휠 확대·드래그 이동 지원
- 버블 클릭 → `/region?poi=…` 상세 이동 (P0 지도와 동일 UX)

**파일**: `src/lib/charts.ts`, `src/app/discover/page.tsx`, `src/app/globals.css`

**키워드**: `P3`, `discover`, `산점도`, `scatter`, `echarts`, `로그축`

---

### [2026-07-25] P3 포지셔닝 산점도 — 사분면별 대표 10개 무작위 표시

**요청**: 전량(8,247개) 표시는 판독이 어려우니, 인기×배출 4개 사분면에서 대표 10개씩 세션마다 랜덤으로 표시.

**변경**:
- 사분면별로 10개씩(총 ≤40개) 무작위 표본 추출 — mulberry32 시드 RNG, 마운트 시 랜덤 시드
- 카드 헤더에 「다시 뽑기」 버튼 → 표본 즉시 교체
- 색상 기준을 대분류 → 사분면(추천=녹색·숨은 명소=파랑·관리 필요=주황·개선 필요=회색)으로 변경, 하단 칩과 동일 팔레트(`QUADRANT_META`)
- 점이 적어져 버블 옆에 POI 이름 라벨 상시 표시(`labelLayout.hideOverlap`)
- 하단 칩 건수는 필터 전체 기준 유지 (표본 아님)

**파일**: `src/lib/charts.ts`, `src/app/discover/page.tsx`, `src/app/globals.css`

**키워드**: `P3`, `discover`, `산점도`, `표본`, `random`, `사분면`

---

### [2026-07-25] P0 AI 인사이트 요약 섹션 추가

**요청**: 검색필터와 6개 KPI 사이에 「AI 인사이트 요약」 섹션 추가. 현재 필터 결과의 요약·시사점을 2~3줄로 표시.

**변경**:
- `nationalInsightSummary()` — 필터 집계(POI 수·방문·배출·집중도·최대 카테고리·저탄소 비율)를 2~3문장으로 생성
- 필터 변경 시 범위 배지·본문이 함께 갱신
- 상단 배너 UI (`.ai-summary`)

**파일**: `src/lib/insights.ts`, `src/app/page.tsx`, `src/app/globals.css`

**키워드**: `P0`, `AI 인사이트`, `요약`, `필터`

---

### [2026-07-25] P0 AI 인사이트 — 전국 저탄소 10 + 코스 3 (필터 무관)

**요청**: AI 인사이트 요약은 검색필터·하단 집계와 무관하게, 전국 전체 조건 기준으로 「17개 시도별 핵심 저탄소 콘텐츠 10개」와 「추천 여행 코스 3 옵션 경로」를 제시. 매 세션·다시 뽑기마다 구성 변화.

**변경**:
- `buildNationalAiInsight(pois, nMonths, seed)` — 저탄소(pc≤1) 후보를 시도 라운드로빈으로 10곳 선발, 시도 내 3스톱 코스 3개 구성
- 필터와 분리된 시드 RNG + 「다시 뽑기」 버튼
- POI 클릭 시 지역 상세로 이동

**파일**: `src/lib/insights.ts`, `src/app/page.tsx`, `src/app/globals.css`

**키워드**: `P0`, `AI 인사이트`, `저탄소`, `여행코스`, `random`

---

### [2026-07-31] 피드백(260728) 반영 — 필터·지도·표기·가이드

**요청**: 대시보드 피드백 중 POI 관련 항목 수정.

**변경**:
- 방문객 유형 필터 제거(전국·카테고리) — 신규 엑셀에 외지인 데이터 없음
- AI 요약 시도 약어 맵(`경상남도→경남`), 카드 최소폭
- 산점도 카드 제목 한 줄화, 중분류 열 nowrap/ellipsis
- MapView: OpenFreeMap 베이스맵 + 전국 지도 클러스터링 + 상세 `easeTo`
- 여행자 유형 → 대분류 가중치로 추천 재정렬 + 화면 설명
- POI 이미지 없음/실패 시 ‘등록된 이미지 없음’ 등 안내 강화

**파일**: `FilterBar.tsx`, `MapView.tsx`, `page/category/discover/guide/region`, `insights.ts`, `PoiPhoto.tsx`, `globals.css`

**키워드**: `피드백`, `클러스터`, `베이스맵`, `여행자유형`, `방문객필터`
